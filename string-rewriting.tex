...
In this \lcnamecref{ch:string-rewriting}, we consider abstract rewriting as a framework for specifying the behavior\fixnote{operational semantics?} of concurrent systems.
This is not, of course, a new idea.
Multiset rewriting\autocite{??}, and other state-transformation models such as Petri nets\autocite{??}, 
[...]\fixnote{fix}
Unlike the application domain for multiset rewriting, we are particularly interested in concurrent systems whose components are arranged in a linear topology and have a monoidal structure.
Given that finite strings over an alphabet $\ialph$ form a monoid, string rewriting is a good match for this structure.

For a broad sketch of string rewriting, consider the finite strings over the alphabet $\Set{a,b}$, and let $\reduces$ be the least compositional\fixnote{compatible?} binary relation over those strings that satisfies the axioms
...
\newthought{The remainder of this \lcnamecref{ch:string-rewriting}} describes the string rewriting framework in more detail~\parencref{sec:string-rewriting:framework} and examines its properties, most importantly concurrent rewritings.
...
\section{String rewriting}\label{sec:string-rewriting:framework}
...
This relation is defined as the least compositional\fixnote{compatible?} relation satisfying a collection of rewriting axioms, chosen on a per-application basis%
, such as the axioms
...
\end{equation}
shown earlier.
More generally, an axiom is any pair of concrete, finite strings, $w \reduces w'$, although axioms of the form $\emp \reduces w'$ are forbidden.
...
so we adopt two compatibility rules.
Together, the rules
\begin{inferences}
  \infer[\jrule{$\reduces$C}\smash{_{\jrule{L}}}]{w_1 \wc w_2 \reduces w'_1 \wc w_2}{
    w_1 \reduces w'_1}
  \and\text{and}\and
  \infer[\jrule{$\reduces$C}\smash{_{\jrule{R}}}]{w_1 \wc w_2 \reduces w_1 \wc w'_2}{
    w_2 \reduces w'_2}
  \,.
\end{inferences}
ensure that the rewriting relation is compatible with the [free]\fixnote{fix} monoidal structure of strings.%
\footnote{Because strings form a monoid, these compatibility rules are equivalent to the unified rule
  \begin{equation*}
    \infer[\jrule{$\reduces$C}]{w_L \wc w \wc w_R \reduces w_L \wc w' \wc w_R}{
      w \reduces w'}
    \:.
  \end{equation*}
...
The $\reduces$ relation describes the rewritings that are possible in a single step: exactly one axiom, perhaps embellished by the compatibility rules.
...
\footnote{Usually written as $\reduces^*$, we instead chose $\Reduces$ for the reflexive, transitive closure because of its similarity with standard process calculus notation for weak transitions, $\Reduces[\alpha]$.
  Our reasons for this choice of notation will become clear in subsequent \lcnamecrefs{ch:ordered-rewriting}.}

Consistent with its [free]\fixnote{fix} monoidal structure, there are two equivalent formulations of this reflexive, transitive closure: each rewriting sequence $w \Reduces w'$ can be viewed as either a list or tree of individual rewriting steps.
...
\subsection{Properties of string  rewriting}

As an abstract rewriting system, string rewriting can be evaluated for several [standard]\fixnote{fix} properties: confluence, termination, and, of particular interest to us, concurrency.
