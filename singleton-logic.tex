...
%
\begin{proof}
  We follow a standard technique for proving the admissibility of a cut principle\autocite{Pfenning:CUT} -- a lexicographic structural induction, first on the structure of the [principal] cut formula, and then on the structures of the given proofs.
  ...
  \item[Commutative cases]
    In the commutative cases, the last inference in one of the proofs introduces a formula other than the cut formula;
    ...
    \begin{gather*}
      ...
      \infer[\lrule{\plus}]{\slseq{A_1 \plus A_2 |- C}}{
        ...
        \infer-[\mathrlap{\jrule{CUT}^B}]{\slseq{A_2 |- C}}{
          ...
          \deduce{\slseq{B |- C}}{\EE}}}
    \end{gather*}
    Notice that there can be no left-commutative cases involving right rules.
  ...
  % \item[Right commutative cases]
    ...
    As another example, one right-commutative case
    ...
    \begin{equation*}
      ...
      \infer[\rrule{\plus}_1]{\slseq{A |- C_1 \plus C_2}}{
        \infer-[\jrule{A-CUT}^B]{\slseq{A |- C_1}}{
          ...
          \deduce{\slseq{B |- C_1}}{\EE_1}}}
    \end{equation*}
    Notice that there can be no right-commutative cases involving left rules because the cut formula is the only antecedent in $\slseq{B |- C}$.
    ...
    There, several right-commutative cases involved left rules acting on antecedents other than the cut formula.
    \qedhere
  \end{description}
\end{proof}
...
%
\begin{theorem}[Non-analytic cut elimination]\label{thm:singleton-logic:seq-calc:cut-elimination}
...
\NewDocumentCommand \idexp { m } { \eta(#1) }
\NewDocumentCommand \idelim { m } { \mathit{ie}(\slof{#1}) }

\begin{align*}
  \idexp(\alpha) &= \fwd \\
  \idexp(\plus*[sub=_{\ell \in L}]{\ell:A_{\ell}}) &= \caseL[\ell \in L]{\ell => \selectR{\ell}} \\
  \idexp(\with*[sub=_{\ell \in L}]{\ell:A_{\ell}}) &= \caseR[\ell \in L]{\ell => \selectL{\ell}}
\end{align*}

\begin{theorem}
  $\slof{A |- \idexp{A} : A}$ for all propositions $A$.
\end{theorem}

\begin{align*}
  \idelim{A |- \spawn{P_1}{P_2} : C} &= \spawn{\idelim{A |- P_1 : B}}{\idelim{B |- P_2 : C}} \\
  \idelim{A |- \fwd : A} &= \idexp{A} \\
  \idelim{A_{\kay} |- \selectR{\kay} : \plus*[sub=_{\ell \in L}]{\ell:A_{\ell}}} &= \selectR{\kay} \\
  \idelim{A |- \caseR[\ell \in L]{\ell => P_{\ell}} : \with*[sub=_{\ell \in L}]{\ell:C_{\ell}}} &= \caseR[\ell \in L]{\ell => \idelim{A |- P_{\ell} : C_{\ell}}} \\
  \idelim{\with*[sub=_{\ell \in L}]{\ell:A_{\ell}} |- \selectL{\kay} : A_{\kay}} &= \selectL{\kay} \\
  \idelim{\plus*[sub=_{\ell \in L}]{\ell:A_{\ell}} |- \caseL[\ell \in L]{\ell => P_{\ell}} : C} &= \caseL[\ell \in L]{\ell => \idelim{A_{\ell} |- P_{\ell} : C}} \\
\end{align*}
