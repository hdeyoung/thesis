...
This Hilbert system can also be viewed as a variant of the sequent calculus.\fixnote{, so we dub it a \emph{semi-axiomatic sequent calculus}.}
...
\item[Proof theory]
  ...
  In adopting the single-antecedent restriction, singleton logic rejects the very idea that contexts have any structure whatsoever; there can be no binary operation to join contexts, so singleton contexts form only the degenerate algebraic structure of a set.\fixnote{Singleton set?  Sets have union.}
...
\item[Type theory]
  ...
  Thus, networks of well-typed processes have a tree topology, as depicted in the neighboring display.\fixnote{fix diagram}%
...
For example, singleton sequent calculus rules for additive disjunction may also be constructed in this way~\parencref[see]{fig:singleton-logic:seq-calc:derive-plus}.\fixnote{fix figure}
%
\begin{figure*}[tbp]
...
\begin{figure}[tbp]
  ...
  \begin{syntax*}
    Propositions &
      A,B,C & p \mid A \plus B \mid \zero \mid A \with B \mid \top
  \end{syntax*}
  \begin{inferences}
    \infer[\jrule{CUT}^B]{\slseq{A |- C}}{
      \slseq{A |- B} & \slseq{B |- C}}
...
In substructural logics and systems built upon them, the logical constants $\top$ and $\zero$ are often problematic because they indiscriminately consume any and all resources placed in front of them.\autocites{Schack-Nielsen:??}{Cervesato+:TCS00}
Interestingly, because the single-antecedent restriction exactly constrains the resources that those logical constants may consume, working in singleton logic is one possible way to sanitize $\top$ and $\zero$.
...
A \emph{verification}, then, is a proof that relies only on the right and left inference rules and the $\jrule{ID}^{p}$ rule for propositional variables $p$\fixnote{Difference between propositional variables and atoms?} -- stated differently, verifications may not contain instances of the $\jrule{CUT}$ or general $\jrule{ID}^A$ rules.

If every proof has a corresponding verification, then we can be sure that neither the $\jrule{CUT}$ nor $\jrule{ID}$ rules play any role in defining the logical co%nnectives.
...
in this \lcnamecref{sec:singleton-logic:hilbert} we also consider a Hilbert-style axiomatization of singleton logic.\fixnote{Should I deemphasize the Hilbert aspects in favor of the semi-axiomatic sequent calculus?}
...
So in a Hilbert-style axiomatization of singleton logic, we will have to content ourselves with a broad interpretation of the term \enquote*{axiom schema} that encompasses zero-premise rules that derive hypothetical judgments.
...
\begin{figure}[tbp]
  ...
  \begin{syntax*}
    Propositions &
      A & p \mid A \plus B \mid \zero \mid A \with B \mid \top
  \end{syntax*}
  \begin{inferences}
    \infer[\jrule{CUT}^B]{\slseq{A |- C}}{
      \slseq{A |- B} & \slseq{B |- C}}
...
As a concrete counterexample, there is no cut-free Hilbert-style proof of the sequent $\slseq{p_2 |- p_1 \plus (p_2 \plus p_3)}$, even though the same sequent is provable using cut:
% \begin{equation*}
...
% \end{equation*}
\begin{equation*}
  \infer[\jrule{CUT}]{\slseq{p_2 |- p_1 \plus (p_2 \plus p_3)}}{
    \infer[\rrule{\plus}'_1]{\slseq{p_2 |- p_2 \plus p_3}}{} &
    \infer[\rrule{\plus}'_2]{\slseq{p_2 \plus p_3 |- p_1 \plus (p_2 \plus p_3)}}{}}
\end{equation*}
...
Similarly, the $\lrule{\plus}$ and $\rrule{\with}$ rules are assigned the proof terms $\caseL[\ell \in L]{\ell => P_{\ell}}$ and $\caseR[\ell \in L]{\ell => P_{\ell}}$.\fixnote{Comments about variable-free combinators?}


\subsection{Non-analytic cut elimination for the singleton Hilbert system}

With proof terms in hand, we can now return to our goal of establishing a \emph{non-analytic} cut elimination \lcnamecref{thm:singleton:hilbert:cutelim} for the singleton Hilbert system.
