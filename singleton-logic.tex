...
This Hilbert system can also be viewed as a variant of the sequent calculus, so we dub it a \emph{semi-axiomatic sequent calculus}.
% Our interest in a Hilbert system is not, however, zoological;
An analysis of its basic metatheory~\parencref{sec:singleton-logic:hilbert:metatheory} begins to suggest the basis of a Curry--Howard interpretation of Hilbert-style proofs as chains of well-typed, asynchronously communicating processes.
...
In substructural logics and systems built upon them, the logical constants $\zero$ and $\top$ are often problematic because they indiscriminately consume any and all resources placed in front of them.\autocites{Schack-Nielsen:??}{Cervesato+:TCS00}
...
In addition to cut elimination, we can also prove identity eliminiation.
...
\end{lemma}
\begin{proof}
  As usual\fixnote{Best reference?}, by induction on the structure of the proposition $A$.
\end{proof}
...
\end{theorem}
\begin{proof}
  As usual\fixnote{Best reference?}, by structural induction on the proof of $\slseq{A |- C}$.
\end{proof}
