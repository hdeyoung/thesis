...
\begin{equation*}
  ...
  \,,
\end{equation*}
where $\cc$ denotes \change[ap]{left-to-right}, end-to-end composition of chains\footnote{\change[ap]{Specifically, the right endpoint of $\chn_1$ will be attached to the left endpoint of $\chn_2$, with the left endpoint of $\chn_1$ and right endpoint of $\chn_2$ becoming the respective left and right endpoints of $\chn_1 \cc \chn_2$.}} and $(\chne)$ denotes the empty chain.
...
Finally, a parallel composition of chains, $\chn_1 \cc \chn_2$, is well-typed only if $\chn_1$ \emph{offers} the same service that $\chn_2$ \emph{uses}, otherwise communication between $\chn_1$ and $\chn_2$ would be mismatched.
...
The mutually coinductive knot can be tied by using $\vdash_{\sig} \sig$ at the top level: $\sig$ is well-formed if $\vdash_{\sig} \sig$ holds \change[ap]{with a finite, non-circular proof}.
\begin{equation*}
