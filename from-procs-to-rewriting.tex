...
\begin{inferences}
  \infer[\jrule{CUT}\smash{^B}]{\slof{A |- \up (\p{A}_1 \fuse \p{A}_2) : C}}{
    \rfocus{\omega_1}{\p{A}_1} & \rfocus{\omega_2}{\p{A}_2} &
    \slof{A |- \omega_1 : B} & \slof{B |- \omega_2 : C}}
  \and
  \infer[\jrule{ID}\smash{^A}]{\slof{A |- \up \one : A}}{}
  \\
  \infer[\rrule{\plus}']{\slof{A_{\kay} |- \selectR{\kay} : \plus*[sub=_{\ell \in L}]{\ell:A_{\ell}}}}{
    \text{($\kay \in L$)}}
  \and
  \infer[\lrule{\plus}]{\slof{\plus*[sub=_{\ell \in L}]{\ell:A_{\ell}} |- {\textstyle \bigwith_{\ell \in L} (\selectR{\ell} \limp \up \p{A}_{\ell})} : C}}{
    \multipremise{\ell \in L}{\rfocus{\omega_{\ell}}{\p{A}_{\ell}} \quad \slof{A_{\ell} |- \omega_{\ell} : C}}}
  \\
  \infer[\rrule{\with}]{\slof{A |- {\textstyle \bigwith_{\ell \in L} (\up \p{A}_{\ell} \pmir \selectL{\ell})} : \with*[sub=_{\ell \in L}]{\ell:B_{\ell}}}}{
    \multipremise{\ell \in L}{\rfocus{\omega_{\ell}}{\p{A}_{\ell}} \quad \slof{A |- \omega_{\ell} : B_{\ell}}}}
  \and
  \infer[\lrule{\with}']{\slof{\with*[sub=_{\ell \in L}]{\ell:B_{\ell}} |- \selectL{\kay} : B_{\kay}}}{
    \text{($\kay \in L$)}}
  \\
  ...
  \infer[\jrule{C-ID}\smash{^A}]{\slcof{A |- \octxe : A}}{}
  \and
  \infer[\jrule{C-PROC}]{\slcof{A |- \omega : B}}{
    \slof{A |- \omega : B}}
\end{inferences}
...
\begin{theorem}
  If $\slcof{A |- \cnf : B}$, then $\slcof{A |- \trconf{\cnf} : B}$.
