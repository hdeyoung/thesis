...
\Cref{sec:correspond:embed:examples} examines the embeddings in the context of the binary counter and \ac{DFA}\fixnote{include \ac{DFA}?}.
...
Embedding the \ac{DFA} process definitions from \cref{??} yields the same coinductively defined propositions as in the functional and object-oriented choreographies from \cref{sec:formula-as-process:nfa-functional,sec:formula-as-process:nfa-oo}:
\begin{align*}
...
\fixnote{Careful with the 0-ary forms because you could end up with $\slof{\zero |- \top : C}$ and \emph{also} $\slof{A |- \top : \top}$.}
