...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
with a clause for $\trconf{P}$ that \change[ap]{remains to be filled in}.
...
%
\begin{marginfigure}[-13.5\baselineskip]
  \begin{align*}
    \trconf{\cnf_1 \cc \cnf_2} &= \trconf{\cnf_1} \oc \trconf{\cnf_2} \\
    \trconf{\cnfe} &= (\octxe) \\
  ...
  %   \\
  %   \embed{\trproc{\defp{p}}} &= \n{\defp{p}}
  \intertext{where $\trproc{P}$ is defined exactly as in \cref{fig:process-embedding:weak-focus-strong-bisim}:}
  %
    \trproc{\spawn{P_1}{P_2}}
      &= \trproc{P_1} \fuse \trproc{P_2} \\
    \trproc{\fwd} &= \one
...
\begin{align*}
  ...
  \,.
\end{align*}
\change[ap]{Notice that $\octx$ is unique, based on properties of the right focus judgment~(see \cref{ch:choreographies}).}
%
\begin{marginfigure}[10\baselineskip]
  \begin{align*}
    \trconf{\cnf_1 \cc \cnf_2} &= \trconf{\cnf_1} \oc \trconf{\cnf_2} \\
    \trconf{\cnfe} &= (\octxe) \\
    \trconf{P} &= \octx \text{ where $\rfocus{\octx}{\trproc{P}}$}
  \intertext{and where $\trsig{\sig}$ and $\trproc{P}$ are defined exactly as in \cref{fig:process-embedding:weak-focus-strong-bisim}:}
  %
    &\mathclap{\begin{aligned}
          \MoveEqLeft[.5]
          \trsig{\sig, (\defp{p} \defd P)} \\
...
Similarly, recall from \cref{sec:formula-as-process:counters-functional} the functional-style process definitions for the binary counter:
\begin{equation*}
  \sig' = \begin{lgathered}[t]
...
%
\noindent
In the judgment $\slof{A |- \p{A}_1 : B}$ and this \lcnamecref{thm:embed:type-props}, $A$ and $B$ are (Curry--Howard interpretations of) singleton propositions that function as session types, where\-as $\p{A}_1$ is a formula-as-process positive ordered proposition that functions as an embedded process expression.

\change[ap]{To construct} \change[cs]{such a type system}, we can simply apply the embedding to session typing rules for process expressions.
For instance, take the session typing rule for a process expression $\spawn{P}{Q}$.
If we apply the embedding to that rule, we arrive at:
\begin{equation*}
  \left\lbrack\,% .\trproc{%
    \raisebox{-0.375\baselineskip}{%
      \infer[\jrule{CUT}\smash{^B}]{\slof{A |- \spawn{P}{Q} : C}}{
        \slof{A |- P : B} & \slof{B |- Q : C}}}\,%
  \right\rbrack% }
  \rightsquigarrow
  \infer{\slof{A |- \begin{array}[t]{@{}c@{}}\trproc{\spawn{P}{Q}}\\=\\\trproc{P} \fuse \trproc{Q}\end{array} : C}}{
    \slof{A |- \trproc{P} : B} & \slof{B |- \trproc{Q} : C}}
\end{equation*}
This embedded rule suggests that ordered conjunctions $\p{A}_1 \fuse \p{A}_2$ be typed with a cut-like rule:
\begin{equation*}
  \infer{\slof{A |- \begin{array}[t]{@{}c@{}}\trproc{\spawn{P}{Q}}\\=\\\trproc{P} \fuse \trproc{Q}\end{array} : C}}{
    \slof{A |- \trproc{P} : B} & \slof{B |- \trproc{Q} : C}}
  \rightsquigarrow
...
\change[ap]{Moreover, just as identity serves as the unit of cut, so does $\one$ serve as the unit of $\fuse$.}
...
\change[ap]{Disallowing $0$-ary alternative conjunctions is not a significant sacrifice, for empty label sets $L$ are extremely unusual in practice.}
...
% \begin{theorem}
%   If $\chorsig{\theta}{\srsig}{\orsig}$ and $\vdash_{\orsig} \orsig$, then $\chorsig{\theta^{\bot}}{\srsig}{\orsig'}$ for some $\orsig'$.
% \end{theorem}
% \begin{proof}
%   \begin{equation*}
%     \infer{\chorsig{\theta}{\srsig_0 , \bigl(w_i^L \wc a \wc w_i^R \reduces w'_i\bigr)_{i \in I}}{\orsig_0 , \bigl(\defp{a} \defd \bigwith_{i \in I} \n{A}_i\bigr)}}{
%       \qimp{\atmR{\octx}_i^L}{\up \bigfuse \theta(w'_i)}{\atmL{\octx}_i^R}{\n{A}_i}}
%   \end{equation*}
% \end{proof}
