\chapter{Ordered bisimilarity}\label{ch:ordered-bisimilarity}

The previous \lcnamecref{ch:??} introduced a formula-as-process view of the focused ordered rewriting framework, and showed how to use it to provide local, message-passing choreographies of the global, string rewriting specifications seen in \cref{ch:string-rewriting}.
The formula-as-process ordered rewriting framework brings (focused) ordered rewriting more in line with process calculi -- [...].
...
In this \lcnamecref{ch:ordered-bisimilarity}, we [...].
...
With the shift from a global, state-transformation view of ordered rewriting put forth in \cref{ch:ordered-rewriting} to the local, formula-as-process view developed in the preceding \lcnamecref{ch:??}, we are now in a position to examine how individual propositions or contexts behave and interact.
Because each proposition has its own local thread of control, we can describe its behavior only to the extent that its behavior can be witnessed by an external observer.
...
As another example, $\atmR{a} \limp (\atmR{c} \pmir \atmL{b})$ and $(\atmR{a} \limp \atmR{c}) \pmir \atmL{b}$ ought to be behaviorally equivalent, intuitively because they are logically equivalent\footnote{$\oseq{\atmR{a} \limp (\atmR{c} \pmir \atmL{b}) \dashv|- (\atmR{a} \limp \atmR{c}) \pmir \atmL{b}}$}.

Following the vast literature on various forms of bisimilarity\footnote{See \textcite{??} for a survey.}, we will develop a notion of \emph{ordered rewriting bisimilarity} on ordered contexts, $\octx \osim \lctx$.
...
However, it will prove to be cumbersome to use only the definition of rewriting bisimilarity to establish that contexts are bisimilar, a problem that is familiar from process calculi notions of bisimilarity.
Therefore, in \cref{sec:??}, we present \emph{labeled bisimilarity}, a sound proof technique for rewriting bisimilarity -- ordered contexts that are labeled bisimilar are also rewriting bisimilar.
Unlike the $\pi$-calculus's labeled bisimilarity, our labeled bisimilarity is surprisingly complete for rewriting bisimilarity -- ordered contexts that are rewriting bisimilar are also labeled bisimilar.
...
\Cref{sec:??} proves that \ac{NFA}-bisimilar states have ordered-bisimilar encodings under the functional choreography described in \cref{sec:??}, which allows us to finally rephrase the choreography's adequacy in a clean and stratified form.
And \cref{sec:??} proves that binary counters are ordered-bisimilar if, and only if, they have the same denotation.

Our rewriting bisimilarity is inspired by and related to the contextual bisimilarity for linear contexts put forth by \textcite{Deng+:MSCS16}.
...
In this \lcnamecref{sec:??}, we define \emph{ordered rewriting bisimilarity}, a notion of observational equivalence for ordered contexts.
...
\footnote{We could equivalently combine these into one monolithic condition: If $\atmR{\lctx}_L \oc \octx \oc \atmL{\lctx}_R \lrframe{\atmR{\lctx}_L}{\simu{R}}{\atmL{\lctx}_R}\Reduces \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$, then $\atmR{\lctx}_L \oc \octx \oc \atmL{\lctx}_R \Reduces\lrframe{\atmL{\lctx}'_L}{\simu{R}}{\atmR{\lctx}'_R} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  However, this formulation is unwieldy, so we prefer the seperate output and input bisimulation conditions.}
...
Expanding slightly, we are given that $\octx \simu{R} \lctx \Reduces \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$, for some context $\lctx$.
Based on the local interaction semantics~\parencref{sec:??}, this means
...
\end{enumerate*}.
For $\simu{R}$ to be a rewriting bisimulation, the context $\octx$ ought to be able to simulate $\lctx$'s eventual output of $\atmL{\lctx}'_L$ and $\atmR{\lctx}'_R$, otherwise the $\simu{R}$-related contexts $\octx$ and $\lctx$ could be distinguished based on their (eventual) output behaviour.
Moreover, the continuations ought to be $\simu{R}$-related as well.
Formally, we ought to have $\octx \Reduces \atmL{\lctx}'_L \oc \octx' \oc \atmR{\lctx}'_R$ (\ie, $\octx$ eventualy outputs $\atmL{\lctx}'_L$ and $\atmR{\lctx}'_R$) and $\octx' \simu{R} \lctx'$, for some context $\octx'$, which is all neatly packaged up as $\octx \Reduces\lrframe{\atmL{\lctx}'_L}{\simu{R}}{\atmR{\lctx}'_R} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.

Input bisimulation is dual to output bisimulation.
Unpacking the given $\atmR{\lctx}_L \oc \octx \oc \atmL{\lctx}_R \lrframe{\atmR{\lctx}_L}{\simu{R}}{\atmL{\lctx}_R}\Reduces \lctx'$, we see that $\octx \simu{R} \lctx$ and $\atmR{\lctx}_L \oc \lctx \oc \atmL{\lctx}_R \Reduces \lctx'$, for some context $\lctx$.
In other words, once provided with the incoming messages $\atmR{\lctx}_L$ and $\atmL{\lctx}_R$, the context $\lctx$ can eventually reach $\lctx'$.
Being $\simu{R}$-related to $\lctx$, the context $\octx$, when provided with the same incoming messages, must be able to reach a context that is $\simu{R}$-related to $\lctx'$, otherwise $\octx$ and $\lctx$ could be distinguished by how they react to $\atmR{\lctx}_L$ and $\atmL{\lctx}_R$.
...
\newthought{The other way} to understand rewriting bisimilarity is by analogy with the asynchronous \ac{CCS}'s notion of weak bisimilarity.\autocites{Amadio+:INRIA96}{Boreale+:IC02}
There, a weak bisimulation can be described as a symmetric relation $\simu{R}$ on processes that satisfies three conditions:%
\footnote{%
  The premises of these conditions are usually stated with strong transitions.
...
When the output and input contexts $\atmL{\lctx}'_L$ and $\atmR{\lctx}'_R$ and $\atmR{\lctx}_L$ and $\atmL{\lctx}_R$ are empty, rewriting bisimilarity's output and input bisimulation conditions amount to a reduction bisimulation property~\parencref{thm:??} and correspond to the second of these \ac{CCS} conditions.

For nonempty output contexts, rewriting bisimilarity's output bisimulation condition corresponds to the first of the asynchronous \ac{CCS} weak bisimilarity's conditions.
As mentioned in \cref{sec:??}, $\lctx \Reduces \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$ functions as an implicit weak output transition from $\lctx$ to $\lctx'$.
Therefore, $\octx \Reduces\lrframe{\atmL{\lctx}'_L}{\simu{R}}{\atmR{\lctx}'_R} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$ is analogous to $P \overset{\smash{\bar{c}}}{\Reduces}\simu{R} Q'$.
...
Because asynchronous \ac{CCS} bisimilarity is a congruence, that condition can be rephrased as \enquote{If $P \simu{R} Q$ and $\bar{c} \mid Q \overset{\tau}{\reduces} Q'$, then $\bar{c} \mid P \overset{\tau}{\Reduces}\simu{R} Q'$} without affecting the resulting bisimilarity.%
\footnote{See \cref{??} for a proof sketch.}
...
In this \lcnamecref{??}, we follow that strategy and develop \vocab{labeled bisimilarity} as a sound -- and, surprisingly, also complete -- proof technique for rewriting bisimilarity.
...
The immediate output, immediate input, and reduction bisimulation conditions are all single-step forms of rewriting bisimilarity's output and input bisimulation conditions~\parencref{??} and reduction bisimulation property~\parencref{??}.
The emptiness bisimulation condition is necessary for labeled bisimilarity to be complete.
A similar condition appears in \textcite{Deng+:MSCS16}.

Emptiness bisimulation is equivalent to requiring that $\octx \simu{R} (\octxe)$ implies $\octx \Reduces (\octxe) \simu{S} (\octxe)$.
...
\begin{equation*}
  ...
  \,.
\end{equation*}
The relation $\simu{R}$ is a labeled bisimulation, but it does not qualify as a rewriting bisimulation because it does not satisfy the more general input bisimulation condition: $\atmR{a} \oc (\atmR{a} \limp (\atmR{c} \pmir \atmL{b})) \lframe{\atmR{a}}{\simu{R}} \atmR{a} \oc ((\atmR{a} \limp \atmR{c}) \pmir \atmL{b})$ does not imply $\atmR{a} \oc (\atmR{a} \limp (\atmR{c} \pmir \atmL{b})) \Reduces\simu{R} \atmR{a} \oc ((\atmR{a} \limp \atmR{c}) \pmir \atmR{b})$.
That would only be possible if $\atmR{a} \oc (\atmR{a} \limp (\atmR{c} \pmir \atmL{b})) \simu{R} \atmR{a} \oc ((\atmR{a} \limp \atmR{c}) \pmir \atmR{b})$.

Although a labeled bisimulation is not a rewriting bisimulation, a slightly weaker statement is nevertheless true: a labeled bisimulation is contained within \emph{some} rewriting bisimulation.
...
The proof of soundness is clean, but it winds through a few \lcnamecrefs{lem:labeled-bisim-union}, however.
...
Having proved the preceding \lcnamecref{lem:??} about framing a single input atom, we can apply it inductively to prove that framing input contexts preserves labeled bisimulations. 
...
Now we use the fact that $\ctxc{\simu{R}}$ is a labeled bisimulation whenever $\simu{R}$ is also one to prove that $\ctxc{\simu{R}}$ is a rewriting bisimulation.
Rewriting bisimilarity therefore contains $\simu{R}$.
...
Nevertheless, this can once again be proved by constructing an appropriate labeled bisimulation, namely the least symmetric relation $\simu{R}$ such that:
...
$(\octxe) \simu{R} (\octxe)$.
The labeled bisimulation shows how the inputs protected by the $\up \dn$ double shifts are trated lazily in establishing the equivalence: the \emph{proposition} $\atmR{c} \pmir \atmL{b}$ is $\simu{R}$-related to the \emph{context} $\atmR{a} \oc \bigl(\up \dn (\atmR{a} \limp \atmR{c}) \pmir \atmL{b}\bigr)$, for example.
...
\begin{proof}
  ...
  Then, it follows from \cref{??} that $\reflc{\simu{R}}$, the reflexive closure of $\simu{R}$, is a labeled bisimulation.
...
\begin{proof}
  ...
  The relation $\osim\osim$ can be shown to be a bisimulation, so rewriting bisimilarity is transitive.
\end{proof}

$\lframe{\atmR{a}}{\osim} \union \osim$ is a labeled bisimulation.
$\lframe{\atmL{a}}{\osim}$ is a labeled bisimulation.
$\lframe{\n{A}}{\osim}$ is a labeled bisimulation.

$\octx \lframe{\atmL{a}}{(\Reduces\lrframe{\atmL{\lctx}''_L}{\osim}{\atmR{\lctx}'_R})} \lctx$

$\octx \lframe{\atmL{a}}{\osim}\Reduces \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.
$\octx \lframe{\atmL{a}}{(\osim\Reduces)} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.

Assume that $\atmR{\lctx}_L \oc \atmL{a} \oc \octx \oc \atmL{\lctx}_R \lrframe{(\atmR{\lctx}_L \oc \atmL{a})}{\osim}{\atmL{\lctx}_R}\Reduces \lctx'$; we must show that $\atmR{\lctx}_L \oc \atmL{a} \oc \octx \oc \atmL{\lctx}_R \lrframe{(\atmR{\lctx}_L \oc \atmL{a})}{\osim}{\atmL{\lctx}_R}\Reduces \lctx'$.
$\atmR{\lctx}_L \oc \atmL{a} \oc \octx \oc \atmL{\lctx}_R \lframe{(\atmR{\lctx}_L \oc \atmL{a})}{(\rframe{\osim}{\atmL{\lctx}_R}\Reduces)} \lctx'$.
$\atmR{\lctx}_L \oc \atmL{a} \oc \octx \oc \atmL{\lctx}_R \lframe{(\atmR{\lctx}_L \oc \atmL{a})}{(\Reduces\osim)} \lctx'$.
$\atmR{\lctx}_L \oc \atmL{a} \oc \octx \oc \atmL{\lctx}_R \Reduces\lframe{(\atmR{\lctx}_L \oc \atmL{a})}{\osim} \lctx'$.
\begin{theorem}
  Rewriting bisimilarity contains $\lframe{\oante}{\osim}$ and $\rframe{\oante}{\osim}$
\end{theorem}
\begin{proof}
  \begin{description}
  \item[Immediate output bisimulation]
    Assume that $\octx \lframe{\oante}{\osim} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$;
    we must show that $\octx \Reduces\mathrel{(\lframe{\oante}{\osim} \union \osim)} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.

    In the case in which $\oante = \n{A}$, the proposition $\n{A}$ can be unified with neither the output atoms $\atmL{\lctx}'_L$ nor $\atmR{\lctx}'_R$.
    So, $\atmL{\lctx}'_L$ is empty and $\lctx'$ begins with $\n{A}$.

    Otherwise, consider the case in which $\oante = \atmL{a}$.
    We distinguish cases on the size of $\atmL{\lctx}'_L$.
    \begin{itemize}
    \item Consider the case in which $\atmL{\lctx}'_L$ is nonempty.
      Then $\oante$ must be some atom $\atmL{a}$, with $\atmL{\lctx}'_L = \atmL{a} \oc \atmL{\lctx}''_L$ for some context $\atmL{\lctx}''_L$, for otherwise $\oante$ would not unify with the output atoms $\atmL{\lctx}'_L$.
      Because $\osim$ is a labeled bisimulation~\parencref{??}, we may appeal to its immediate output bisimulation property after framing off $\atmL{a}$ and deduce that $\octx \lframe{\atmL{a}}{(\Reduces\lrframe{\atmL{\lctx}''_L}{\osim}{\atmR{\lctx}'_R})} \lctx$.
      Reduction is closed under framing, so we conclude that $\octx \Reduces\lrframe{\atmL{\lctx}'_L}{\osim}{\atmR{\lctx}'_R} \lctx$.
      % 
    \item Consider the case in which $\atmL{\lctx}'_L$ is empty.
      Then $\oante$ must be some atom $\atmL{a}$, with $\atmL{\lctx}'_L = \atmL{a} \oc \atmL{\lctx}''_L$ for some context $\atmL{\lctx}''_L$, for otherwise $\oante$ would not unify with the output atoms $\atmL{\lctx}'_L$.
      Because $\osim$ is a labeled bisimulation~\parencref{??}, we may appeal to its immediate output bisimulation property after framing off $\atmL{a}$ and deduce that $\octx \lframe{\atmL{a}}{(\Reduces\lrframe{\atmL{\lctx}''_L}{\osim}{\atmR{\lctx}'_R})} \lctx$.
      Reduction is closed under framing, so we conclude that $\octx \Reduces\lrframe{\atmL{\lctx}'_L}{\osim}{\atmR{\lctx}'_R} \lctx$.
    ...
    Because $\oante$ [...], it does not unify with $\atmL{\lctx}'_L$, the context $\atmL{\lctx}'_L$ must be empty.
    
    $\octx \lframe{\oante}{\osim}\Reduces \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.
    $\octx \lframe{\oante}{(\osim\Reduces)} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.
    $\octx \lframe{\oante}{(\Reduces\osim)} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.
    $\octx \Reduces\lframe{\oante}{\osim} \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$.
  \end{description}

  \begin{description}
  \item[Immediate output bisimulation]
    Assume that $\octx \lframe{\n{A}}{\osim} \lctx = \atmL{\lctx}'_L \oc \lctx' \oc \atmR{\lctx}'_R$;
    it suffices to show that $\octx \Reduces\lrframe{\atmL{\lctx}'_L}{\lframe{\n{A}}{\osim}}{\atmR{\lctx}'_R} \lctx$.

    The proposition $\n{A}$ cannot be unified with $\atmL{\lctx}'_L$, so $\atmL{\lctx}'_L$ must be empty.
    By output bisimulation, $\octx \lframe{\n{A}}{(\Reduces\rframe{\osim}{\atmR{\lctx}'_R})} \lctx' \atmR{\lctx}'_R$.
    Because reduction is closed under framing, $\octx \Reduces\rframe{\lframe{\n{A}}{\osim}}{\atmR{\lctx}'_R} \lctx$.

  \item[Immediate input bisimulation]
    Assume that $\octx \lframe{\n{A}}{\osim} \lctx$ and $\ireduces{\atmR{\lctx}_L \oc #1 \oc \atmL{\lctx}_R}{\lctx}{\lctx'}$;
    we must show that

    $\lctx = \n{A} \oc \lctx_0$ and $\ireduces{#1 \oc \atmL{\lctx}_R}{\lctx_0}{\lctx'_0}$ and $\lctx' = \n{A} \oc \lctx'_0$.
    $\octx \oc \atmL{\lctx}_R \lframe{\n{A}}{(\Reduces\osim)} \lctx'$.
    $\octx \oc \atmL{\lctx}_R \Reduces\lframe{\n{A}}{\osim} \lctx'$.

  \item[Reduction bisimulation]
    Assume that $\octx \lframe{\n{A}}{\osim}\reduces \lctx'$;
    we must show that $\octx \Reduces\mathrel{(\lframe{\n{A}}{\osim} \union \osim)} \lctx'$.

    \begin{itemize}
    \item
      Consider the case in which $\octx \lframe{\n{A}}{(\osim\reduces)} \lctx'$.
      $\octx \lframe{\n{A}}{(\Reduces\osim)} \lctx'$.
      Reduction is closed under framing, and so $\octx \Reduces\lframe{\n{A}}{\osim} \lctx'$, as required.

    \item Consider the cae in which the reduction arises from $\n{A}$ -- that is, $\octx \lframe{\n{A}}{\osim} \n{A} \oc \atmL{\lctx}_R \oc \lctx'_R$ and $\ireduces{#1 \oc \atmL{\lctx}_R}{\n{A}}{\lctx'_0}$ and $\lctx' = \lctx'_0 \oc \lctx'_R$.
      By immediate output bisimulation, $\octx \lframe{\n{A}}{(\Reduces\lframe{\atmL{\lctx}_R}{\osim})} \n{A} \oc \atmL{\lctx}_R \oc \lctx'_R$.
      Because reduction is closeed under framing, $\octx \Reduces\lframe{(\n{A} \oc \atmL{\lctx}_R)}{\osim}\reduces \lctx'$.
      $\octx \Reduces\lframe{(\n{A} \oc \atmL{\lctx}_R)}{\osim}\reduces \lctx'$.
    \end{itemize}
  \end{description}
\end{proof}
...
Recall from \cref{??} the conjecture that bisimilar \ac{NFA} states have bisimilar encodings and vice versa.
This will allow us to rephrase soundness and completeness of the \ac{NFA} choreography in a properly stratified way: $\atmR{a} \oc \defp{q} \reduces_{\orsig}\osim \defp{q}'$ if, and only if, $q \nfareduces[a]\asim q'$.

More precisely, we will prove the following \lcnamecref{??}.
\begin{restatable*}[
...
\begin{proof}
  ...
  \item
    ...
    The first part of this \lcnamecref{lem:??} shows that states are encoded by latent propositions, and so we may conclude that, in fact, $\defp{q}'_a \osim \defp{q}'$.

  \item 
    Consider the case in which $\atmR{a} \oc \defp{q} \reduces \defp{q}'_a \Reduces\osim \defp{q}'$ for some state $q'_a$ that $a$-succeeds $q$.
    Because states are encoded by latent propositions, $\defp{q}'_a \osim \defp{q}'$ for some state $q'_a$ that $a$-succeeds $q$, as required.
  %
  \qedhere
  \end{itemize}
  % 
...
\begin{proof}
  ...
  \begin{itemize}[itemsep=\dimexpr\itemsep+\parsep\relax, parsep=0em, listparindent=\parindent]
    \item
      To prove that bisimilar \ac{NFA} states have bisimilar encodings -- \ie, that $q \asim s$ implies $\defp{q} \osim \defp{s}$ -- we will show that the relation $\mathord{\simu{R}} = \Set{(\defp{q}, \defp{s}) \given q \asim s}$ is a labeled bisimulation up to reflexivity and, by \cref{thm:bisim-technique-up-to-refl}, is included in rewriting bisimilarity.
      Notice that $\simu{R}$ is symmetric.
      \begin{description}
      \item[Immediate output bisimulation]
      ...
      \item[Reduction bisimulation]
        ...
        The reduction bisimulation property holds vacuously because states are encoded as latent propositions, and so $\defp{s}$ does not reduce~\parencref{lem:nfa-latent}.
    ...
    \item
      To prove the converse -- that states with bisimilar encodings are themselves bisimilar -- we will show that the relation $\mathord{\simu{R}} = \Set{(q,s) \given \defp{q} \osim \defp{s}}$, which relates states if they have rewriting-bisimilar encodings, is \iac{NFA} bisimulation and is therefore included in bisimilarity.
      ...
      \item[Finality bisimulation] Let $q$ and $s$ be states with bisimilar encodings, and assume that $q$ is a final state;
        ...
        By definition of the encoding, $\atmR{\eow} \oc \defp{q} \reduces \atmR{F}(q) = \symacc$.
        Because $q$ and $s$ have bisimilar encodings, it follows from input bisimilarity that $\atmR{\eow} \oc \defp{s} \Reduces\osim \atmR{F}(q)$.
        An appeal to \cref{??} yields $\atmR{F}(q) = \atmR{F}(s)$, from which we deduce that $s$ is also a final state.
      %
      \qedhere
      \end{description}
    \end{itemize}
...
Recall from \cref{sec:??} the object-oriented choreography of the binary counter:
\begin{equation*}
  \orsig =
  \begin{lgathered}[t]
...
After all, the only two operations that we have on counters are increment and head-unary normalization (or decrement), and both of these are reflected in the denotation.
...
We can state and prove that counters with equal denotations are bisimilar, and conversely, bisimilar counters have equal denotations.
\begin{restatable}[
...
\begin{lemma}
  If $\octx = \atmL{\octx}'_L \oc \octx' \oc \atmR{\octx}'_R$, then:
  \begin{itemize}[nosep]
  \item $\ainc{\octx}{n}$ only if $\atmL{\octx}'_L$ and $\atmR{\octx}'_R$ are empty; and
  \item $\adec{\octx}{n}$ only if $\atmL{\octx}'_L$ is empty and either:
    \begin{itemize}[nosep]
    \item $n=0$ and $\atmR{\octx}'_R = \atmR{z}$ and $\octx'$ is empty;
    \item $n > 0$ and $\atmR{\octx}'_R = \atmR{s}$ and $\ainc{\octx'}{n-1}$; or
    \item $\atmR{\octx}'_R$ is empty.
    \end{itemize}
  \end{itemize}
\end{lemma}
\begin{proof}
...
Using this \lcnamecref{??}, we may prove the correspondence between denotation and bisimilarity:
\thmoocounterbisim*
\begin{proof}
  ...
  \item[Immediate output bisimulation]
    ...
    \item
      ...
      A straightforward induction on the derivation of $\ainc{\lctx}{n}$ shows that $\atmL{\lctx}'_L$ and $\atmR{\lctx}'_R$ must both be empty.
  ...
  \item[Immediate input bisimulation]
    ...
    \item 
      ...
      It follows from $\adec{}{}$-preservation~\parencref{??} that $\adec{\lctx'}{n}$; we conclude that $\octx \Reduces\simu{R} \lctx'$, as required.
    \end{itemize}
