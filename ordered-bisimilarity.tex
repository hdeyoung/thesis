...
\begin{proof}
  ...
  \begin{description}
  \item[Immediate input bisimulation]
    Assume that $q \asim s$ and $\ireduces{\atmR{\lctx}_L \oc #1 \oc \atmL{\lctx}_R}{\nfa{q}}{\lctx'}$;
    we must show that $\atmR{\lctx}_L \oc \nfa{s} \oc \atmL{\lctx}_R \Reduces\refl*{\simu{R}}^{-1} \lctx'$.
    \begin{itemize}
    \item Consider the case in which: $\atmR{\lctx}_L = \atmR{a}$ for some $a \in \ialph$; $\atmL{\lctx}_R = \octxe$; and $\lctx' = \bigwith_{q' \mid q \nfareduces[a] q'} \nfa{q}'$.
      Unfortunately, $\lctx' = \bigwith_{q' \mid q \nfareduces[a] q'} \nfa{q}'$ is not in the relation $\refl{\simu{R}}$, except trivially.
    \item Consider the case in which $\atmR{\lctx}_L = \atmR{\emp}$; $\atmL{\lctx}_R = \octxe$; and $\lctx' = \one$ because $q$ is a final state.
        Because $q \asim s$, we know that $s$ is also a final state.
        Therefore, $\atmR{\emp} \oc \nfa{s} \Reduces \one \refl*{\simu{R}} \one = \lctx'$.
      \end{itemize}
    \end{itemize}
  %
  \item[Reduction bisimulation]
