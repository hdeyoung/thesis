...
\change[ap]{In addition to grounding concurrency in logic, ordered rewriting provides the logical connectives, which function as a richer, more convenient set of primitives for describing concurrent systems.}
...
Ordered rewriting is then obtained as exactly the decomposition-centric fragment of the refactored sequent calculus~\parencref{sec:ordered-rewriting:unfocused}.
...
Like its string rewriting cousin, the ordered rewriting framework allows disjoint segments of an ordered context to be rewritten independently;
concurrency arises within ordered rewriting when the various interleavings of independent rewritings are treated indistinguishably~\parencref{sec:ordered-rewriting:concurrency}.
...
Filling in the $\oseq{\octx |- A}$ premises with the $\jrule{ID}^A$ rule, we arrive at the derivable \change[ap]{\emph{modus ponens}-like} rules
...
% Instead, we want ordered rewriting to be a syntactic procedure, withou 
 and the accompanying proof search would take ordered rewriting too far afield from traditional, syntactic notions of string and multiset rewriting.\footnote{\change[ic]{For similar reasons, the $\omega$ rewriting system proposed by \textcite{Cervesato+Scedrov:IC09} restricts the linear implication rule to have no minor premise.}}
...
\end{marginfigure}%
\change[ap]{This operation creates a proposition that can be \enquote{flattened} into $\octx$.
The proposition $\bigfuse \octx$ is equivalent to $\octx$ in the following sense:}
%
\begin{lemma}
...
\change[ap]{Recall from \cref{ch:string-rewriting} that confluence requires} all states with a common ancestor, \ie, states $\octx'_1$ and $\octx'_2$ such that $\octx'_1 \secudeR\Reduces \octx'_2$, to be joinable, \ie, $\octx'_1 \Reduces\secudeR \octx'_2$.
...
\begin{proof}
  ...
  In the latter case, \cref{lem:ordered-rewriting:reduction} establishes $\card{\octx_{i+1}}_{\star} < 0$, which is impossible because $\card{}_{\star}$ is a measure \change[ap]{of the number of logical connectives in a context} and is hence nonnegative.
\end{proof}
...
\newthought{Focusing, as developed by} \textfootcite{Andreoli:JLC92}, provides just the right coarse-grained decomposition through its complementary inversion and chaining strategies for proof search.
...
This idea of using focusing to increase the granularity of rewriting steps dates back to, at least, the \acf{CLF}\autocite{Watkins+:CMU02} and was later streamlined in linear logic by \textfootcite{Cervesato+Scedrov:IC09}.
\Textfootcite{Simmons:CMU12} has studied a focused ordered rewriting framework, though in a somewhat different formulation than the one we present here -- \change[ap]{\citeauthor{Simmons:CMU12} uses small-step focusing rules rather than \textfootcite{Zeilberger:POPL08}}.


\newthought{The ordered propositions} are now polarized into positive and negative classes, or \vocab{polarities}\autocite{Andreoli:JLC92}, according to the invertibility of their sequent calculus rules; two \enquote*{shift} operators, $\dn$ and $\up$, mediate between the two classes.
\begin{align*}
  % Positive props. &
...
Notice that we use the same metavariables for these polarized ordered contexts as we did for the unpolarized contexts of the preceding (unfocused) ordered rewriting framework -- the intended \change[ic]{form}, either polarized or not, will always be clear from context.
...
\change[ap]{The positive proposition $\bigfuse \octx$ can be \enquote{flattened} into $\octx$.
The $\dn$ shifts are needed to incorporate negative propositions $\n{A}$ within the positive structure of $\bigfuse \octx$.}
...
Following \textfootcite{Zeilberger:POPL08}, each of these judgments can be read as a function that provides a form of extended decomposition -- the in-focus proposition is decomposed beyond its immediate subformulas, until subformulas of the opposite polarity are reached.
...
\change[cs]{Our goal in presenting this is twofold.
First, these embeddings demonstrate the expressive power of focusing when shifts are present.
In particular, by embedding the unfocused ordered rewriting framework in the focused framework, we can see that the focused framework is more expressive than the unfocused one.
Second, we will make use of the embedding of weakly focused ordered rewriting in \cref{ch:correspond} to arrive at a stronger theorem than we would otherwise get.}
...
% % $\embedb*{}$, from unpolarized propositions to negative propositions and positive atoms;
% $\embedbn*{}$ and $\embedbp*{}$, from unpolarized propositions to negative and positive propositions, respectively.
% $\embedbn{A}$ and $\embedbp{A}$ produce negative and positive polarizations of $A$ that insert a $\dn \up$ shift in front of every proper, nonatomic subformula of $A$.
% In addition, $\embedbn{A}$ prepends an $\up$ shift whenever the top-level connective of $A$ has positive polarity, whereas $\embedbn{A}$ prepends a $\dn$ shift whenever $A$ is not atomic.
% % $\embedb{A}$ produces either a positive atom or a negative polarization of $A$, according to wheter $A$ is atomic.
%
% \begin{marginfigure}
% \begin{equation*}
%   \begin{aligned}
%     \embedbp{A} &= \begin{cases*}
%                     \p{a} & if $A = a$ \\
%                     \dn \embedbn{A} & otherwise
%                \end{cases*}
%     \\
%     \embedbn{A} &= \begin{cases*}
%                     \up (\embedbp{A_1} \fuse \embedbp{A_2}) & if $A = A_1 \fuse A_2$ \\
%                     \up \one & if $A = \one$ \\
%                     \embedbp{A_1} \limp \up \embedbp{A_2} & if $A = A_1 \limp A_2$ \\
%                     \up \embedbp{A_2} \pmir \embedbp{A_1} & if $A = A_2 \pmir A_1$ \\
%                     \up \embedbp{A_1} \with \up \embedbp{A_2} & if $A = A_1 \with A_2$ \\
%                     \top & if $A = \top$
%                   \end{cases*}
%     \\
%     \embedb{A} &= \begin{cases*}
%                    \p{a} & if $A = a$ \\
%                    \embedbn{A} & otherwise
%                  \end{cases*}
%     \\
%     \embedb{\octx} &= \begin{cases*}
%                        \embedb{\octx_1} \oc \embedb{\octx_2} & if $\octx = \octx_1 \oc \octx_2$ \\
%                        (\octxe) & if $\octx = \octxe$ \\
%                        \embedb{A} & if $\octx = A$
%                      \end{cases*}
%   \end{aligned}
...
\change[ap]{The $\embedp*{\p{A}}$ translation turns a (weakly focused) positive proposition $\p{A}$ into either an atomic proposition or a negative proposition by judiciously inserting or removing shifts.
The $\embedn*{\n{A}}$ translation turns a (weakly focused) negative proposition $\n{A}$ into a (strongly focused) negative proposition compositionally.}
