\chapter[Choreographies: A formula-as-process interpretation of ordered rewriting]{Choreographies: A formula-as-process\\interpretation of ordered rewriting}\label{ch:formula-as-process}\label{ch:choreographies}
...
This \lcnamecref{ch:formula-as-process} takes three significant steps away from pure specification and toward programming, using the focused ordered rewriting framework of the previous \lcnamecref{ch:ordered-rewriting} as a stepping stone.
...
\change[ap]{Intuitively, processes have a negative polarity because they wait for an external signal to begin computation.
Messages have a positive polarity because they exist without external input.
This kind of external/internal duality is typical of negative and positive propositions.\autocite{Zeilberger:CMU09}}
...
\end{equation*}%
%
\begin{margintable}[-0.5\baselineskip]
  \begin{center}
    \begin{tabular}{@{}r@{\enspace}>{\itshape}l@{}}
      $\p{\atmR{a}} \limp \n{B}$ & receive message $\p{\atmR{a}}$ from the left \\
      $\n{B} \pmir \p{\atmL{a}}$ & receive message $\p{\atmL{a}}$ from the right \\
...
\end{marginfigure}%
This operation \change[ap]{is the same as the one defined in \cref{fig:ordered-rewriting:polarized-bigfuse}}, except that atom directions are preserved.
...
Sending a message is instead accomplished by spawning a process that is itself only a message: $\p{\atmL{a}} \fuse \p{B}$, $\p{B} \fuse \p{\atmR{a}}$, $\p{\atmR{a}} \fuse \p{B}$, and $\p{B} \fuse \p{\atmL{a}}$ \change[ap]{-- sending to the left, sending to the right, self-sending at the left, and self-sending at the right, respectively --} are all possible but built from other propositional forms.
...
%     \lfocus{\atmR{\octx}_L}{\n{B}}{\atmL{\octx}_R}{\p{C}}}
% \end{equation*}
The other rules for the left-focus judgment remain fundamentally unchanged, save for the fact that the left- and right-hand contexts now contain only atoms of the complementary direction.\footnote{\change[ic]{That the left- and right-hand contexts now contain only atoms might seem like a harsh restriction, but these judgments are for focusing.  The rewriting judgment will allow other negative propositions to appear via its compatibility rule.}}
\end{itemize}
...
\end{proof}
\noindent
\change[ap]{In addition, this \lcnamecref{thm:formula-as-process:no-recapture-outputs} shows that no interactions among $\atmL{\octx}_L$, nor among $\atmR{\octx}_R$, are possible.  This is because $\atmL{\octx}_L$ and $\atmR{\octx}_R$ consist only of messages, which are passive and only trigger computation when delivered to processes.}
...
Moreover, in \cref{ch:correspond}, we will present a correspondence between ordered propositions and the not-yet-introduced singleton proofs~\parencref{ch:singleton-logic,ch:process-chains}, which will turn out to be more direct if we retain the general $\p{A} \fuse \p{B}$ form for ordered conjunctions.
...
Because our coinductively defined propositions are \emph{not} generative\footnote[][0.75\baselineskip]{\change[ap]{Generative type abstraction, such as that of ML modules, would ensure that an abstract type is distinguished from its representation -- isomorphic types are not equal.}}, there is not much difference between their equirecursive and isorecursive treatments.\autocite{Amadio+Cardelli:TOPLAS93}
...
As examples, both $\atmL{a} \oc \atmL{b} \oc \n{C}$ and $\atmL{a} \oc \n{C} \oc \atmL{b}$ \change[ap]{immediately} output $\atmL{a}$ to the left (and nothing to the right).
More precisely, the former \change[ap]{immediately} outputs $\atmL{a} \oc \atmL{b}$, whereas the latter does not \change[ap]{immediately} output $\atmL{b}$ at all.
...
\change[ap]{For example, as we saw above, $\atmL{a} \oc (\atmL{b} \pmir \atmL{b}) \oc \atmL{b}$ does not immediately output $\atmL{a} \oc \atmL{b}$.
But $\atmL{a} \oc (\atmL{b} \pmir \atmL{b}) \oc \atmL{b}$ does \emph{weakly} output $\atmL{a} \oc \atmL{b}$, by virtue of the rewriting $\atmL{a} \oc (\atmL{b} \pmir \atmL{b}) \oc \atmL{b} \reduces \atmL{a} \oc \atmL{b}$.}
...
\change[ap]{In terms of a functional reading of the judgment, this judgment is a function on contexts $\octx$ that returns the contexts $\atmR{\octx}_L$, $\atmL{\octx}_R$, and $\octx'$.}\footnote{\change[ap]{Stated in terms of an input/output mode, $\octx$ is the input to the judgment, whereas the input contexts $\atmR{\octx}_L$ and $\atmL{\octx}_R$ and the context $\octx'$ are outputs of the judgment.}}
...
\footnote[][-\baselineskip]{Notice that it is quite possible in this rule for both $\atmR{\octx}_L$ and $\atmL{\octx}_R$ to be empty and for the judgment to express the input of no messages at all.
...
The difference is that, unlike the reduction judgment, the input transition judgment is enriched with several other rules.%
\footnote{Another difference is the input/output mode of this input transition, as compared to that of a reduction.
...
\change[ic]{As an example, these observations show that a string rewriting axiom $a \wc a \reduces w'$ has no valid choreography -- if $a$ takes a process role, then $\defp{a} \oc \defp{a} \reduces \theta(w')$ violates the second observation; if $a$ takes a message role, then $\atmL{a} \oc \atmL{a} \reduces \theta(w')$ or $\atmR{a} \oc \atmR{a} \reduces \theta(w')$ violates the first observation.}
...
To give a formal description of choreographing specifications, we define a judgment\footnote{\change[ic]{We will frequently omit the string rewriting alphabet $\sralph$ from this judgment, as the intended alphabet is always clear from context.}} $\chorsig{\theta}{_{\sralph} \srsig}{\orsig}$ that, when given a string rewriting specification $(\sralph, \srsig)$ and a role assignment $\theta$, yields formula-as-process definitions $\orsig$ that make string rewriting under $\srsig$ and formula-as-process ordered rewriting under $\orsig$ bisimilar, if such definitions exist:
% \begin{equation*}
%   ??
% \end{equation*}
...
\footnote[][-2.25\baselineskip]{This procedure could be made deterministic by preferring one side over the other, but we refrain from doing so because the choice of side to prefer is completely arbitrary.}
...
\begin{table*}[tbp]
  ...
  \hfill\begin{tabular}{@{}l@{\qquad}l@{\qquad}l@{}}
    \toprule
    \emph{Axioms, $\srsig$} &
...
\begin{equation*}
  ...
  \,.
\end{equation*}
Upon solving the remaining constraints for the other coinductively defined propositions, $\defp{d}$ and $\defp{b}'_0$~\parencref{tbl:formula-as-process:deriving-functional-choreography},\ 
%
\begin{table*}[tbp]
...
One problem with these \change[ic]{alternative} choreographies, however, is that, without a $\atmR{z}$ or $\atmR{s}$ response message, there is no way to observe the counter's state.
...
\section{Example: Choreographing \aclp*{NFA}}\label{sec:formula-as-process:nfa}
...
\marginnote{$
  \theta =
    \!\begin{lgathered}[t]
      ...
      {} \union \Set{ \symacc \mapsto \atmR{\symacc} , \symrej \mapsto \atmR{\symrej} }
    \end{lgathered}
$}
...
\begin{marginfigure}[-1.5\baselineskip]
  ...
  \begin{equation*}
    \orsig =
    \begin{lgathered}[t]
      \bigl(\defp{q}_0 \defd (\atmR{a} \limp \up \dn \defp{q}_0) \begin{array}[t]{@{{}\with{}}l@{}} (\atmR{b} \limp (\up \dn \defp{q}_0 \with \up \dn \defp{q}_1)) \\ (\atmR{\eow} \limp \up \atmR{\symrej})\bigr) \,, \end{array} \\
...
In the specific instance of this functional choreography of the \ac{NFA}, \cref{cor:formula-as-process:choreography-adequacy}, the adequacy of the choreographing procedure, can be reduced to the following \lcnamecref{cor:chor-nfa-adequacy} -- a result that relates the string rewriting specification to the choreography.
...
\marginnote{$
    \theta' =
      ...
      \end{lgathered}
$}
the role assignment that maps each input symbol $a \in \ialph$ and the end-of-word marker, $\eow$, to coinductively defined propositions, $\defp{a}$ and $\defp{\eow}$, respectively; each state $q \in Q$ to a left-directed message, $\atmL{q}$; and the \change[ic]{acceptance and} rejection symbols, $\symacc$ and $\symrej$, to right-directed messages, \change[ic]{$\atmR{\symacc}$ and} $\atmR{\symrej}$.
...
As for the functional choreography, we may then establish a shortcut adequacy theorem for this object-oriented choreography as a \lcnamecref{cor:formula-as-process:nfa-oochor-adequacy} of earlier results.
Composing this \lcnamecref{prop:formula-as-process:nfa-oochor-sr} with \cref{thm:nfa-adequacy-string-rewriting}, the adequacy of formula-as-process choreographies with respect to their underlying string rewriting specifications, we arrive at:
...
A closer examination of the preceding counterexample reveals that the states $q_1$ and $s_1$, while not equal, are in fact bisimilar~\parencref{sec:finite-automata:bisimilarity}.
...
Second, in the following \lcnamecref{ch:ordered-bisimilarity}, we will develop a richer notion of propositional equivalence, \emph{ordered rewriting bisimilarity}.
In applying it to the functional choreography of \acp{NFA}~\parencref{sec:formula-as-process:nfa-functional}, we will see that \ac{NFA} states that are \ac{NFA}-bisimilar have state encodings that, while not necessarily equal, are always rewriting-bisimilar; the converse will also hold.
